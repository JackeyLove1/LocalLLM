...
ExecutorResult QwenExecutor::Prefill(const std::vector<std::int64_t>& prompt_token_ids) {
  LOG(INFO) << "Starting prefill with " << prompt_token_ids.size() << " prompt tokens.";
  ResetState();
...
GenerationResult QwenExecutor::Generate(
  ...
  LOG(INFO) << "Entering generation loop: max_new_tokens=" << params.max_new_tokens
  ...
  for (std::int64_t step = 0; step < params.max_new_tokens && !stop_requested; ++step) {
    ...
    if (params.stream) {
      ...
      if (params.print_token_ids) {
        LOG(INFO) << "Generated token id: " << next_token;
      }
    }

    if (next_token == config_.eos_token_id) {
      generated_token_ids_.push_back(next_token);
      LOG(INFO) << "Stopping generation after EOS token at step " << (step + 1) << '.';
      break;
    }
    for (const auto& stop : params.stop_strings) {
      if (!stop.empty() && generated_text.size() >= stop.size() &&
        ...
        stop_requested = true;
        LOG(INFO) << "Stopping generation after stop string match at step " << (step + 1) << '.';
        break;
      }
    }
  ...
  LOG(INFO) << "Generation completed: prompt_tokens=" << prompt_token_ids_.size()
            << ", generated_tokens=" << generated_token_ids_.size()
            << ", total_tokens=" << all.size();
